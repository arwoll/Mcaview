...
while any(strcmp(tok, {'#C', '@A', '@AMCA', '@B', '@AMCS1', '@AMCS2'}))
    if strcmp(tok, '#C') 
        if ~isempty(regexp(nextline, 'aborted'))
            aborts = aborts + 1;
        ...
        if ~MCA_channels(mcadev)
            ...
            mcatext = textscan(specfile, '%f32', 'whitespace', ' \b\t\\');
            mcadata.(MCA_fields{mcadev}) = mcatext{1};
...
if columns ~= length(headers)
    errors = add_error(errors, 2, sprintf(['Badly formed data file: #L line has %d labels, but \n' ...
        '%d values were detected in data line'], length(headers), columns));
end
